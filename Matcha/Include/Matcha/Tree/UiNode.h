...
public:
    ...
    // -- Widget bridge (framework-internal) --

    /// @brief Return the underlying QWidget, if any.
    /// @return nullptr by default. Overridden by container/widget nodes.
    ///
    /// @warning **Framework-internal.** Business-layer code should use the typed
    /// property accessors on WidgetNode subclasses (SetText, SetEnabled, etc.)
    /// rather than accessing the raw QWidget. Direct QWidget manipulation bypasses
    /// the notification system and may cause inconsistencies.
    /// For C ABI / cross-language bindings, use WidgetNode::NativeHandle() instead.
