...
void LineEditNode::BindText(Observable<std::string>& source)
{
    _textBinding = PropertyBinding<std::string>(source,
        [this](const std::string& val) { SetText(val); });
}

void LineEditNode::BindTextTwoWay(Observable<std::string>& source)
{
    BindText(source);
    _textChangeSub = matcha::ScopedSubscription(*this, this, "TextChanged",
        [&source](matcha::EventNode& /*sender*/, matcha::Notification& notif) {
            auto& tc = *notif.UnsafeAs<matcha::fw::TextChanged>();
            source.Set(std::string(tc.Text()));
        });
}

void LineEditNode::UnbindAll()
{
    _textBinding.Release();
    _textChangeSub.Release();
}
