...
#include <Matcha/Theming/Token/DtfmTokenModel.h>
...
auto DtfmTypeToString(DtfmType t) -> std::string
{
    switch (t) {
    case DtfmType::Color:      return "color";
    case DtfmType::Dimension:  return "dimension";
    case DtfmType::Transition: return "transition";
    case DtfmType::FontFamily: return "fontFamily";
    case DtfmType::FontWeight: return "fontWeight";
    case DtfmType::Number:     return "number";
    case DtfmType::String:     return "string";
    }
    return "string";
}

auto DtfmTypeFromString(std::string_view s) -> std::optional<DtfmType>
{
    if (s == "color")      return DtfmType::Color;
    if (s == "dimension")  return DtfmType::Dimension;
    if (s == "transition") return DtfmType::Transition;
    if (s == "fontFamily") return DtfmType::FontFamily;
    if (s == "fontWeight") return DtfmType::FontWeight;
    if (s == "number")     return DtfmType::Number;
    if (s == "string")     return DtfmType::String;
    return std::nullopt;
}
...
void WriteValue(std::ostringstream& os, const DtfmToken& token, int level, int indent);

void WriteDimensionValue(std::ostringstream& os, const DtfmDimension& dim,
...
void WriteTransitionValue(std::ostringstream& os, const DtfmTransition& tr,
                          int level, int indent)
{
    os << '{' << Nl(indent);
    WriteIndent(os, level + 1, indent);
...
void WriteValue(std::ostringstream& os, const DtfmToken& token, int level, int indent)
{
    ...
    } else if (std::holds_alternative<DtfmDimension>(token.value)) {
        WriteDimensionValue(os, std::get<DtfmDimension>(token.value), level, indent);
    } else if (std::holds_alternative<DtfmTransition>(token.value)) {
        WriteTransitionValue(os, std::get<DtfmTransition>(token.value), level, indent);
...
void WriteToken(std::ostringstream& os, const DtfmToken& token,
                int level, int indent)
{
    WriteIndent(os, level, indent);
    ...
    WriteIndent(os, level + 1, indent);
    os << "\"$type\":" << Sep(indent);
    WriteString(os, DtfmTypeToString(token.type));
    os << ',' << Nl(indent);
...
void WriteGroup(std::ostringstream& os, const DtfmTokenGroup& group,
                int level, int indent)
{
    WriteIndent(os, level, indent);
...
auto ParseDtcgToken(const std::string& name,
                    const JsonParser::JsonObject& obj) -> std::optional<DtfmToken>
{
    ...
    auto tfmType = DtfmTypeFromString(*typeStr);
    if (!tfmType) { return std::nullopt; }
    ...
    if (const auto* strVal = valueVal->AsString()) {
        ...
        if (*tfmType == DtfmType::Dimension) {
            ...
            token.value = dim;
        } else if (*tfmType == DtfmType::Transition) {
            DtfmTransition tr;
...
auto ParseGroup(const std::string& name,
                const JsonParser::JsonObject& obj) -> DtfmTokenGroup
{
    DtfmTokenGroup group;
    group.name = name;
...
void FlattenGroup(const DtfmTokenGroup& group, const std::string& prefix,
                  std::unordered_map<std::string, DtfmToken>& out)
{
...
auto DtfmSerializer::Export(const DtfmTokenFile& file, int indent) -> std::string
{
...
auto DtfmSerializer::Import(std::string_view json) -> std::optional<DtfmTokenFile>
{
    JsonParser parser;
    parser.src = json;
...
auto DtfmSerializer::Flatten(const DtfmTokenFile& file)
    -> std::unordered_map<std::string, DtfmToken>
{
    std::unordered_map<std::string, DtfmToken> result;
    for (const auto& group : file.groups) {
