...
> **On concrete design decisions.** This document was authored without a dedicated visual designer on the team. Wherever it specifies concrete design choices — color palette values, spacing scales, border radii, shadow elevations, animation curves, icon styles, typography pairings, and similar visual particulars — those decisions were made by engineers (Me, Nekomiya) following intuition, industry benchmarks, and cognitive-science heuristics, **not** by a professional designer applying a deliberate visual identity process. Every such decision is open to discussion and revision; none should be treated as final. In fact, producing these concrete visual specifications is precisely one of the core deliverables that a designer should provide (see Appendix F for a full deliverables overview). The framework-level architecture — token taxonomy, state machine model, component API contracts — is engineered and stable; the pixel-level aesthetics layered on top of it are provisional and await proper design ownership.
